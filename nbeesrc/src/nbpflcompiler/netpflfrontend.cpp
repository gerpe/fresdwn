...
//Creates an fsa with only the startprotoco in case of there is the exctractfield action but the filtering expression has not been specified
...
EncapFSA *NetPFLFrontEnd::VisitFilterTermExpr(PFLTermExpression *expr)
{
	//here, no mir code has been generated
	EncapGraph &protoGraph = m_GlobalInfo.GetProtoGraph();
 ...
 */
	...
	if(!expr->MandatoryTunnels())
		fsa = fsaBuilder.BuildEncapFSA(protocol, expr->Clone()); 
	else
		fsa = fsaBuilder.BuildEncapFSATunnels(protocol,expr->Clone());
...
EncapFSA *NetPFLFrontEnd::VisitFilterExpression(PFLExpression *expr, bool fieldExtraction, NodeList_t toExtract)
{
	...
	if(expr != NULL)
	{
		...
		case PFL_TERM_EXPRESSION:
			fsa = VisitFilterTermExpr((PFLTermExpression*)expr);  
			break;
...
//  if(!stateFrom->isFinal() and stateFrom->GetInfo()==NULL) //FIXME: provo sta cosa ma ho paura che sia una cazzata
 // 	 m_MIRCodeGen->JumpStatement(JUMPW, protoFilterInfo.FilterFalseLbl );
  ...
  std::map<string, EarlyCodeEmission_t> early_protos;							/* map of protocols for which we need to emit early the before code
																				 ...
																				 */	
  map<string,EncapFSA::State*> nextStates; /*[icerrato]*/ 						/* map storing, for each protocol which requires the early emission of the before section, 
  																				 * the state related to that protocol
  																				 */
  
  std::list<EncapFSA::Transition*> trlist = fsa->getTransExitingFrom(stateFrom);


  for (std::list<EncapFSA::Transition*>::iterator tr_iter = trlist.begin();
       tr_iter != trlist.end();
       ++tr_iter) {  
    ...
    if((*tr_iter)->getIncludingET() == NULL) { 
      ...
      if (to_s == statesInfo.end()) {
        ...
        if ((*tr_iter)->IsComplementSet())
            ...
            ++sym)
          {
            ...
            if( to_s->first->GetInfo() == NULL &&  //[icerrato]
               (  (*sym).second->FirstExecuteBefore || (fieldExtraction))
               )
               {
 	             toStateMap[(*sym).second->Name] = setupEarlyCode(&early_protos,
                                                               (*sym).second,
                                                               (*to_s).second->getLabelComplete(),
                                                               fieldExtraction
    	            );
    	         nextStates[(*sym).second->Name] = &(*to_s->first); 
    	       }
            else
      ...
      SymbolProto *proto = (*(trinfo.first.begin())).second;
      ...
      if (et_check == et_tohandle.end()){
        ...
        et_tohandle.insert(std::pair<EncapFSA::ExtendedTransition*, SymbolLabel*>(et,
                                                                                  tmp_label)
                           );
        toStateMap.insert(std::pair<string,SymbolLabel*>(proto->Name,
                                                         tmp_label)
                          );
      }
    }
  ...
  // 7) emit the ExtendedTransition code
  if (!et_tohandle.empty()) {
    ...
    for (std::map<EncapFSA::ExtendedTransition*, SymbolLabel*>:: iterator et_iter = et_tohandle.begin();
         ...
         ++et_iter) {
      m_MIRCodeGen->CommentStatement("[debug] ET loop separator");
      ...
      GenerateETCode(et_iter->first, statesInfo, protoFilterInfo.FilterFalseLbl, &ETcodeGen, fieldExtraction);
  ...
  // 8) emit the early code for those states that need it
      ...
      ++i)
  {
    ...
    EncapFSA::State *nextState = (nextStates.find(i->first))->second;
    if(fieldExtraction)
    {
    	m_MIRCodeGen->CommentStatement("EARLY FORMAT SECTION FOR PROTOCOL: " +protocol->Name);
    	GenerateFormatCode(protocol,fsa, protoFilterInfo,nextState,statesInfo);
    }
...
void NetPFLFrontEnd::GenerateFormatCode(SymbolProto *protocol,EncapFSA *fsa,FilterCodeGenInfo &protoFilterInfo, EncapFSA::State *stateFrom, const sInfoMap_t statesInfo)
{		
	sInfoMap_t::const_iterator infoPtr = statesInfo.find(stateFrom);
	StateCodeInfo *curStateInfo = infoPtr->second;

	m_NetVMIRGen->LowerHIRCode(&(m_GlobalSymbols.GetProtoCode(protocol)),
				                  	protocol,
				                   "emitting early format for protocol " + protocol->Name);

	m_MIRCodeGen->CommentStatement("DONE emitting the proto format");

    // [dark] try and handle multifield. Code by mgaudesi, trunk rev 2680 e 2683.
	/*if(protocol->proto_MultiField) { [icerrato] it should be useless
		SymbolProto *proto_mf = protocol;
		m_MIRCodeGen->CommentStatement(string("MultiField - memorizzazione numero campi estratti ") + proto_mf->Name);
		FieldsList_t::iterator f = proto_mf->proto_MultiFields.begin();
		for (; f != proto_mf->proto_MultiFields.end(); f++)
		{	
			//store the number of fields extracted
			m_MIRCodeGen->GenStatement(m_MIRCodeGen->BinOp(IISTR, m_MIRCodeGen->TermNode(LOCLD, proto_mf->proto_FieldCount), m_MIRCodeGen->TermNode(PUSH, (*f)->Position)));
		}
	}*/
	...
	if(!stateFrom->isFinal())
	{
		...
		//now, we have to emit also the code related to the encapsulation :-(
		//FIXME:[icerrato] per ora suppongo che due stati consecutivi multiprotocollo non esistano e probabilmente non esistono
	
		// cycle on all transitions and prepare the data structures
	  	std::map<string, SymbolLabel*> toStateMap; // map of the states reachable from this one
	  	
	  	/* map of the ET that must be handled. The mapped value is a
			 temporary label at which position all the code for the ET will be
			 generated.*/
	  	std::map<EncapFSA::ExtendedTransition*, SymbolLabel*> et_tohandle;
		std::list<EncapFSA::Transition*> trlist = fsa->getTransExitingFrom(stateFrom);
		  ...
		  CodeList *encapCode = m_GlobalSymbols.NewCodeList(true);
		  HIRCodeGen encapCodeGen(m_GlobalSymbols, encapCode);
		  NetPDLParser.ParseEncapsulation(protocol, toStateMap, fsa, stateFrom->isAccepting(),
				                          protoFilterInfo.FilterFalseLbl, protoFilterInfo.FilterTrueLbl, encapCodeGen);
		  // emit the encapsulation code
		  m_NetVMIRGen->LowerHIRCode(encapCode, "emitting encapsulation for proto inside " + curStateInfo->toString());
		  m_MIRCodeGen->CommentStatement("DONE emitting the proto encapsulation");
...
void NetPFLFrontEnd::ETCodegenCB_newlabel(unsigned long id, string proto_field_name, void *ptr){
  ...
  info->cur_field_sym = info->instance->m_GlobalSymbols.LookUpProtoFieldByName(proto_name,
                                                                               field_name);
...
void NetPFLFrontEnd::ETCodegenCB_range(RangeOperator_t r, uint32 sep, void *ptr){
  ...
  StmtIf *ifStmt = codegen->IfStatement(
    codegen->BinOp(rangeOpToIROp(r),
                   codegen->UnOp(IR_CINT,
                                 codegen->TermNode(IR_FIELD, info->cur_field_sym)
                     ),
                   codegen->TermNode(IR_ICONST, codegen->ConstIntSymbol(sep))
      )
    );
...
void NetPFLFrontEnd::ETCodegenCB_punct(RangeOperator_t r, const map<uint32,unsigned long> &mappings, void *ptr){
  ...
  HIRStmtSwitch *swStmt = codegen->SwitchStatement(
    codegen->UnOp(IR_CINT,
                  codegen->TermNode(IR_FIELD, info->cur_field_sym)
      )
    );
  ...
  for(map<uint32,unsigned long>::const_iterator i = mappings.begin();
    ...
    subCode.JumpStatement(/*JUMPW, */m->second); 
...
/*[icerrato] UNUSED
HIRNode* NetPFLFrontEnd::extractComplexIRExpr(PFLExpression *expr, HIRCodeGen *codeGen){
  nbASSERT(expr->GetProtocol() != NULL, "Cannot extract an IR expression from a complex PFLExpression based on different protocols.");
  if (expr->GetProtocol() == NULL)
    return NULL;
  ...
  switch (expr->GetType()){
  case PFL_TERM_EXPRESSION:
    return static_cast<HIRNode*>(((PFLTermExpression*)expr)->GetIRExpr()->Clone());
    break;

  case PFL_UNARY_EXPRESSION:
    nbASSERT( ((PFLUnaryExpression*)expr)->GetOperator() == UNOP_BOOLNOT, "Do we know other unary operators? Why didn't I get the memo?");
    tmp1 = extractComplexIRExpr( ((PFLUnaryExpression*)expr)->GetChild(), codeGen);
    if (tmp1 == NULL)
      return NULL;
    return codeGen->UnOp(IR_NOTB, tmp1);
    break;

  case PFL_BINARY_EXPRESSION:
    switch ( ((PFLBinaryExpression*)expr)->GetOperator() ){
    case BINOP_BOOLAND:
      opKind = IR_ANDB;
      break;
    case BINOP_BOOLOR:
      ...
      break;
    default:
      nbASSERT(false, "Unhandled ((PFLBinaryExpression*)expr)->GetOperator()");
      return NULL;
    }
    tmp1 = extractComplexIRExpr( ((PFLBinaryExpression*)expr)->GetLeftNode(), codeGen);
    tmp2 = extractComplexIRExpr( ((PFLBinaryExpression*)expr)->GetRightNode(), codeGen);
    if (tmp1 == NULL || tmp2 == NULL)
      return NULL;
    return codeGen->BinOp(opKind, tmp1, tmp2);
    break;

  default:
    nbASSERT(false, "Unhandled expr->GetType()");
    return NULL;
  }
}*/
...
void NetPFLFrontEnd::ETCodegenCB_special(RangeOperator_t r, std::string val, void *ptr){
 	...
 	PDLFieldType fieldType = info->cur_field_sym->FieldType;
 	...
 	if((r==MATCH)||(r==CONTAINS)){//we have to match a regular expression
 		...
 		switch(fieldType){
			...
			{
	 			SymbolFieldVarLen *varlenFieldSym = static_cast<SymbolFieldVarLen*>(info->cur_field_sym);
			...
			{
 				SymbolFieldTokEnd *tokEndFieldSym = static_cast<SymbolFieldTokEnd*>(info->cur_field_sym);
			...
			break;
                        case PDL_FIELD_LINE:
 			{
				SymbolFieldLine *lineFieldSym = static_cast<SymbolFieldLine*>(info->cur_field_sym);
			...
			{
	 			SymbolFieldPattern *patternFieldSym = static_cast<SymbolFieldPattern*>(info->cur_field_sym);
			...
			{
	 			SymbolFieldEatAll *eatAllFieldSym = static_cast<SymbolFieldEatAll*>(info->cur_field_sym);
			...
			break;
	 		case PDL_FIELD_FIXED:
			{		
	 			SymbolFieldFixed *fixedFieldSym = static_cast<SymbolFieldFixed*>(info->cur_field_sym);	
