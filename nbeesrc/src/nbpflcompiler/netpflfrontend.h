...
private:
	...
	void GenerateFormatCode(SymbolProto *protocol, EncapFSA *fsa,FilterCodeGenInfo &protoFilterInfo, EncapFSA::State *stateFrom, const sInfoMap_t statesInfo);
         ...
         */
        void GenerateETCode(EncapFSA::ExtendedTransition *et, const sInfoMap_t stateMappings, SymbolLabel *defaultFailure, HIRCodeGen *codeGenerator, bool fieldExtraction);

        // the following group of types and methods is used by the previous method
        struct ETCodegenCB_info_t {
          NetPFLFrontEnd *instance;
          SymbolField *cur_field_sym;
	...
	EncapFSA *GenFieldExtract(NodeList_t &protocols, SymbolLabel *trueLabel, SymbolLabel *falseLabel, EncapFSA *fsa /*icerrato*/);

	EncapFSA *VisitFilterTermExpr(PFLTermExpression *expr);
	
	EncapFSA *VisitNullExpr(); //[icerrato]
