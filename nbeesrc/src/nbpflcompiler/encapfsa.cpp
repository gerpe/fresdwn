...
//Needed to implement the field extraction
...
//Needed to implement the the field extraction
...
//[icerrato] needed to implement the keyword "tunneled"
EncapFSA *EncapFSABuilder::BuildEncapFSATunnels(SymbolProto *target, PFLTermExpression *expr)
{

	uint32 index = expr->GetHeaderIndex(); 
	//if the filter is "proto%n tunneled" and n>1, the automaton is the same of the one for the filter "proto%n"
	if(index>1) 
		return BuildEncapFSA(target, expr);	
		
	//now, we have to classify each protocol of the protocol encapsulation graph
	AssignProtocolLayer assigner(m_Graph);
	assigner.Classify();

	EncapFSA *fsa = new EncapFSA(m_Alphabet);
	EncapFSA::Alphabet_t empty;
	
	double targetLevel = target->Level;
	
	//creating final not accepting state
	...
	Node *irExpr = NULL;
	irExpr = expr->GetIRExpr();
	//if irExpr is still NULL, the target has not a predicate
	
	map<EncapFSA::State*,EncapFSA::Transition*> toFail;		//this map contains those transitions leading to the final not accepting state 
	set<pair<EncapFSA::State*,SymbolProto*> > transitions;	//this set contains those transitions leading to a state of a lower level than the one of the target
	
	//creates a normal state and a final accepting state for the target protocol
	EncapGraph::GraphNode *targetNode = m_Graph.GetNode(target);
	//final accepting state
	EncapStIterator_t targetAcceptingState = this->AddState(*fsa, *targetNode,false);  
	fsa->setFinal(targetAcceptingState);
   	fsa->setAccepting(targetAcceptingState);
   	fsa->AddTrans(targetAcceptingState, targetAcceptingState, empty, true, NULL);
	//normal state
	EncapFSA::State *targetState = NULL;
	if(index!=1)
	{
		//the normal state is needed only if the header indexing is 0
		EncapStIterator_t targetStateIt = this->AddState(*fsa, *targetNode,true);  
		//we have to check if the target has a self loop
		bool selfLoop = false;
		list<EncapGraph::GraphNode*> &predecessors = (targetNode)->GetPredecessors();
		...
		EncapLabel_t label = std::make_pair<SymbolProto*, SymbolProto*>(target, target);
		targetState = &(*targetStateIt);
		//if the target protocol has a self loop, now we link the two states related to the target 
		if(selfLoop) 
		{
			//there is the self loop on the target node
			EncapTrIterator_t fail = fsa->AddTrans(targetStateIt, finalNotAccept, label, true, NULL);	
			toFail[targetState] = &(*fail);			
			if(irExpr==NULL)	
			{
				fsa->AddTrans(targetStateIt, targetAcceptingState, label, false, NULL);
			}
			else
				fsa->addExtended(targetStateIt, label, targetAcceptingState, finalNotAccept, expr);
		} 
		else
		{
			EncapTrIterator_t fail = fsa->AddTrans(targetStateIt, finalNotAccept, empty, true, NULL);	
			toFail[targetState] = &(*fail);
		}
	}

	m_Graph.SortRevPostorder(*targetNode);
	//iterate over the nodes
	for (EncapGraph::SortedIterator i = m_Graph.FirstNodeSorted(); i != m_Graph.LastNodeSorted(); i++)
	{
		SymbolProto *current = (*i)->NodeInfo;
		double currentLevel = current->Level;
		bool currentIsTarget = false;
	
		EncapFSA::State *currentState = NULL;
		if(current==target)
		{
			//we have to link the predecessor to a state related to the target, depending on its level and the target level	
			currentIsTarget = true;		
		}
		else
		{
			EncapStIterator_t currentStateIt = this->AddState(*fsa, **i,false);  
			currentState = &(*currentStateIt);
			if((*i)==&startNode)
				fsa->SetInitialState(currentStateIt);
		}
		
		list<EncapGraph::GraphNode*> &predecessors = (*i)->GetPredecessors();
		for (list<EncapGraph::GraphNode*>::iterator p = predecessors.begin() ;p != predecessors.end(); p++)
		{
			SymbolProto *predecessor = (*p)->NodeInfo;
			EncapLabel_t label = std::make_pair<SymbolProto*, SymbolProto*>(predecessor, current);
			
			//consider the case where the current and the predecessor are the same node
			if(current==predecessor)
			{
				if(!currentIsTarget)
				{
					//we have to add the self loop
					EncapStIterator_t state = this->AddState(*fsa, **p,false);  
					fsa->AddTrans(state, state, label, false, NULL);
					if(toFail.count(&(*state))==0)
					{
						EncapTrIterator_t fail =	fsa->AddTrans(state, finalNotAccept, label, true, NULL);
						toFail[&(*state)] = &(*fail);
					}
					else
					{
						EncapFSA::Transition *fail = (toFail.find(&(*state)))->second;
						fail->AddLabel(label);
					}
				}
				//if the current is target, the link already exists
				continue;
			}
		
			//the current and the predecessor are different
			
			bool predecessorIsTarget = false;
			EncapFSA::State *predecessorState = NULL;
			if(predecessor==target)
				predecessorIsTarget=true;
			else
			{
				EncapStIterator_t predecessorStateIt = this->AddState(*fsa, **p,false);  
				predecessorState = &(*predecessorStateIt);
				
			}
		
			double predecessorLevel = predecessor->Level;
			if(predecessorLevel>=currentLevel)
			{
					if(currentIsTarget)
					{
						//we can perform the link
						EncapStIterator_t predecessorStateIt = fsa->GetStateIterator(predecessorState);
						if(irExpr)
							//we use an extended transition
							fsa->addExtended(predecessorStateIt, label, targetAcceptingState, finalNotAccept, expr); 
						else
							//we use a normal transition
							fsa->AddTrans(predecessorStateIt,targetAcceptingState,label);
							
						if(toFail.count(&(*predecessorState))==0)
						{
							EncapTrIterator_t fail =	fsa->AddTrans(predecessorStateIt, finalNotAccept, label, true, NULL);
							toFail[&(*predecessorState)] = &(*fail);
						}
						else
						{
							EncapFSA::Transition *fail = (toFail.find(&(*predecessorState)))->second;
							fail->AddLabel(label);
						}
					}
					else
					{
						if(predecessorIsTarget)
							predecessorState = targetState;
						if(predecessorLevel<targetLevel)
						{	
							//the link must be performed
							EncapStIterator_t currentStateIt = fsa->GetStateIterator(currentState);
							EncapStIterator_t predecessorStateIt = fsa->GetStateIterator(predecessorState);
							fsa->AddTrans(predecessorStateIt, currentStateIt, label, false, NULL);
							if(toFail.count(&(*predecessorState))==0)
							{
								EncapTrIterator_t fail =	fsa->AddTrans(predecessorStateIt, finalNotAccept, label, true, NULL);
								toFail[&(*predecessorState)] = &(*fail);
						...
						//we store the predecessorState and the targetProtocol
						if((index!=1) || !predecessorIsTarget) //[icerrato]FIXME: sono praticamente sicuro che questo if sia giusto, ma potrebbero esserci dei problemi.. l'automa generato pero' sembra ok
						{
								pair<EncapFSA::State*,SymbolProto*> p(predecessorState,current);
			...
			else
			{
				...
				//we link the predecessor to the current. In case of the current is the target, the link is performed to the normal state and only in case of the index is !=1 
				if((!currentIsTarget && !predecessorIsTarget) || (index!=1))
				{
					if(currentIsTarget)
	...
	PRINT_DOT(fsa, "after the first part", "buildencapfsatunnels_first");
	
	if(transitions.size()>0)
		AddSecondPartTunneled(fsa,transitions,targetAcceptingState,toFail, finalNotAccept, target, irExpr, expr);

	//a further check is needed in case of the header indexing is one. In fact it is possible that some states have not exiting transitions. Then this state should be linked to the final not accepting state, in order to ensure the correctness of the automaton
	if(index==1)
	{
		EncapFSA::StateList_t states = fsa->GetAllStates();
		for(EncapFSA::StateList_t::iterator s = states.begin(); s != states.end();s++)
		{
			EncapStIterator_t aux = fsa->GetStateIterator(*s);
			if((fsa->getTransExitingFrom(*s)).size() == 0)
			{
				fsa->AddTrans(aux, finalNotAccept, empty, true, NULL);
			}
		}
	}
	
	PRINT_DOT(fsa, "before BuildEncapFSATunnels return", "buildencapfsatunnels");

	return fsa;
}

//[icerrato] needed to implement the keyword "tunneled"
EncapFSA *EncapFSABuilder::AddSecondPartTunneled(EncapFSA *fsa, set<pair<EncapFSA::State*,SymbolProto*> > transitions, EncapStIterator_t targetAcceptingState, map<EncapFSA::State*,EncapFSA::Transition*> toFail, EncapStIterator_t finalNotAccept, SymbolProto *target, Node *irExpr, PFLTermExpression *expr)
{	
	map<SymbolProto*,EncapFSA::State*>states;
	set<pair<EncapFSA::State*,EncapFSA::State*> >transitionsAdded;
	
	EncapGraph::GraphNode *targetNode = m_Graph.GetNode(target);

	for(set<pair<EncapFSA::State*,SymbolProto*> >::iterator it = transitions.begin(); it != transitions.end();it++)
	{
		pair<EncapFSA::State*,SymbolProto*> currentPair = *it;
		EncapFSA::State *fromState = currentPair.first;
		nbASSERT(fromState!=NULL,"fromState cannot be NULL!");
		EncapStIterator_t fromStateIt = fsa->GetStateIterator(fromState);
		
		SymbolProto *toProtocol = currentPair.second;
		nbASSERT(toProtocol!=NULL,"toProtocol cannot be NULL!");
		EncapFSA::State *toState = NULL;
		EncapGraph::GraphNode *toNode = m_Graph.GetNode(currentPair.second);
		if(states.count(toProtocol)==0)
		{
			EncapStIterator_t toStateIt = this->AddState(*fsa, *toNode,true);  
			toState = &(*toStateIt);
			states[toProtocol] = toState;
		}
		else
		{
			toState = (states.find(toProtocol))->second;
		}
	
		EncapLabel_t label = std::make_pair<SymbolProto*, SymbolProto*>(fromState->GetInfo(),toProtocol);
		EncapStIterator_t toStateIt = fsa->GetStateIterator(toState);
		fsa->AddTrans(fromStateIt, toStateIt, label, false, NULL);
		if(toFail.count(currentPair.first)==0)
		{
			EncapTrIterator_t fail = fsa->AddTrans(fromStateIt, finalNotAccept, label, true, NULL);
			toFail[&(*fromState)] = &(*fail);
		}
		else
		{
			EncapFSA::Transition *fail = (toFail.find(&(*fromState)))->second;
			fail->AddLabel(label);
		}
		
		//now we have to link toState and the targetAcceptingState
		if(m_Graph.ExistPaths(*targetNode, *toNode))
		{
			multimap<EncapGraph::GraphNode*,EncapGraph::GraphEdge> mp = m_Graph.GetMapNodeEdges();
			for (EncapGraph::OnPathIterator i = m_Graph.FirstNodeOnPath(); i != m_Graph.LastNodeOnPath(); i++)
			{
				bool currentIsTarget = false;
			
				EncapFSA::State *newState = NULL;
				SymbolProto *destProto = (*i)->NodeInfo;
				if(destProto==target)
				{
					newState = &(*targetAcceptingState);
					currentIsTarget = true;
				}
				else
				{
					if(states.count(destProto)==0)
					{
						EncapStIterator_t newStateIt = (this->AddState(*fsa, **i, true));
						newState = &(*newStateIt);
						states[destProto] = newState;
					}
					else
						newState = (states.find(destProto))->second;
				}
				nbASSERT(newState!=NULL,"There is an error!");
			
				pair<multimap<EncapGraph::GraphNode*,EncapGraph::GraphEdge>::iterator, multimap<EncapGraph::GraphNode*,EncapGraph::GraphEdge>::iterator> pmp;
				pmp = mp.equal_range(*i);	
				for (multimap<EncapGraph::GraphNode*,EncapGraph::GraphEdge>::iterator mi = pmp.first; mi != pmp.second; ++mi)
				{
					SymbolProto *fromProto = (*mi).second.From.NodeInfo;
					if(fromProto == target)
						continue; //FIXME: [icerrato] non sono sicuro che vada bene. Ho verificato e molto probabilmente e' giusto
						
					EncapLabel_t label = std::make_pair<SymbolProto*, SymbolProto*>(fromProto,destProto);		
					
					EncapFSA::State *stateSrc = NULL;

					if(states.count(fromProto)==0)
					{
						EncapStIterator_t fromStateIt = (this->AddState(*fsa, (*mi).second.From, true));
						stateSrc = &(*fromStateIt);
						states[fromProto] = stateSrc;
					}
					else
						stateSrc = (states.find(fromProto))->second;
					nbASSERT(stateSrc!=NULL,"There is an error!");
			
					EncapStIterator_t fromStateIt = fsa->GetStateIterator(stateSrc);
					EncapStIterator_t newStateIt = fsa->GetStateIterator(newState);
					
					pair<EncapFSA::State*,EncapFSA::State*> pairState(stateSrc,newState);
					if(transitionsAdded.count(pairState)==0)
					{			
						transitionsAdded.insert(pairState);
					
						if(currentIsTarget && irExpr) 
						{
							//extended transition
							fsa->addExtended(fromStateIt, label, newStateIt, finalNotAccept, expr);
						}
						else
						{
							//no extended trasition
							fsa->AddTrans(fromStateIt, newStateIt, label, false, NULL);	
						}		
						EncapFSA::Transition *failTr = (toFail.find(stateSrc)!=toFail.end())? toFail.find(stateSrc)->second : NULL;
						if(failTr==NULL){
							EncapTrIterator_t badTransition = fsa->AddTrans(fromStateIt, finalNotAccept, label, true, NULL);
							toFail[stateSrc] = &(*badTransition);
						}
						else
							failTr->AddLabel(label);
					}	
						
				}	//end for on predecessors
			}//end of for		
		}
		else
			fsa->removeState(toState);
			
		PRINT_DOT(fsa, "BuildEncapFSATunnels after a new protocol", "buildencapfsatunnels_after_new_protocol");
	}

	return fsa;
}

EncapFSA *EncapFSABuilder::BuildEncapFSA(SymbolProto *target, PFLTermExpression *expr)
{
	EncapFSA *fsa = new EncapFSA(m_Alphabet);
	EncapGraph::GraphNode &startNode = m_Graph.GetStartProtoNode();	
	
	//[icerrato]
	EncapFSA::State *auxState = NULL;
	EncapFSA::Transition *auxTransition = NULL;
	uint32 index = expr->GetHeaderIndex(); //in the default case, index == 0
    ...
    // [DARK] handle the predicate (if any)
    Node *irExpr = NULL;
    // if(expr && expr->GetType() == PFL_TERM_EXPRESSION)
	irExpr = expr->GetIRExpr();
	/* After these instructions, if irExpr == NULL, then no
	* predicate was specified on the commandline, therefore no
	* ExtendedTransition is needed.
	*/
	...
	assert(targetNode);
     ...
     const bool cur_target_is_inside_a_loop = m_Graph.SortRevPostorder(*targetNode);
	for (EncapGraph::SortedIterator i = m_Graph.FirstNodeSorted(); i != m_Graph.LastNodeSorted(); i++)
	{
        /* What is this partialFailureState? When we
         * have loops in the encapsulation graph, like
         * IP in IP, some protocols might appear more
         * than once. For instance, applying the
         * filter 'ip.src == X', even if the external
         * IP layer does not match the predicate, an
         * arbitrary internal one might.
         * 
         * In this case, partialFailureState stores a
         * pointer to the the cloned, non accepting
         * state, otherwise it just points to the
         * global failure state of the automaton.
         */
         
        const bool clone_must_be_created = cur_target_is_inside_a_loop && ((*i) == targetNode) && irExpr && (index == 0);
        EncapStIterator_t partialFailureState = ( clone_must_be_created ?
                                                 this->AddState(*fsa, **i) :
                                                 finalNotAccept);
                                                
        EncapStIterator_t newState = this->AddState(*fsa, **i,
                                                    (clone_must_be_created) ? true : false);                                       
...

		{
			...
			{//ip%1 and ip are equivalent
            	...
            	auxState = &(*newState);
            	EncapTrIterator_t newTransition = fsa->AddTrans(newState, finalNotAccept, empty, true, NULL);	
		...
		for (list<EncapGraph::GraphNode*>::iterator p = predecessors.begin();
                         ...
                         p++)
		{
                            //PRINT_DOT(fsa, "tightloop1", "tightloop1");
			EncapStIterator_t predecessorState = this->AddState(*fsa, **p);
                            /* If the predecessor is an accepting state, usually I do not have to draw transitions and such;
                             *
                             * If this is not the case, I will handle later the few things to do, just skip it for now.
                             */
                            if ((*p) == targetNode && !(irExpr&&(index == 0)))
                              continue;


			EncapLabel_t label = std::make_pair<SymbolProto*, SymbolProto*>((*p)->NodeInfo, (*i)->NodeInfo);
			bool pred2cur_found = false, pred2fail_found = false;
			for(EncapFSA::TransIterator j = fsa->FirstTrans(); j != fsa->LastTrans(); j++)
			{
				if((*(*j).ToState()).GetInfo() == (*newState).GetInfo() &&
                                       (*(*j).FromState()).GetInfo() == (*predecessorState).GetInfo())
                                      // if the transition 'j' goes from the predecessor to the current state,
                                      // no need to add it again: set a flag to remember it
                                      pred2cur_found = true;

				if((*(*j).FromState()).GetInfo() == (*predecessorState).GetInfo() &&
                                       (*(*j).ToState()) == *finalNotAccept) {
                                      // if the transition 'j' is the "default transition" from the precedessor towards the
                                      // final non-accepting state
                                      (*j).AddLabel(label);
                                      pred2fail_found = true;
                                    }
			}
              
			if(!pred2cur_found // no transition from the predecessor 'p' to the current state ('newState') was found
                               || clone_must_be_created) // or if this is the 'shadow' clone of the accepting state
			{
                              if ((*i) == targetNode && irExpr && ((index == 1)||(index==0))){                                  
                                // If we get here with a predicate, create two transitions
                                // (one towards success and one, with the negated predicate, towards FAIL)
                                // that will be backed up by the same ExtendedTransition.
                                // This method will take care of both cases
                                fsa->addExtended(predecessorState, label, newState, partialFailureState, expr); 
                                }
                              else
                                // the predicate must be linked to the transition only if 'newstate' is among targets...
                                fsa->AddTrans(predecessorState, newState, label, false,
                                              (((*i) == targetNode&& ((index==1)||(index==0))) ? expr : NULL) );
                            }
                            if(!pred2fail_found){
                              // create the transition towards the fail state, if it was not found already
                              fsa->AddTrans(predecessorState, finalNotAccept, label, true, NULL);
			}
		}
		
	} /* for (EncapGraph::SortedIterator i = m_Graph.FirstNodeSorted(); i != m_Graph.LastNodeSorted(); i++) */
	
	if((index>1)&& cur_target_is_inside_a_loop)
		CompleteFSA(fsa, auxState, index, targetNode, empty,finalNotAccept, auxTransition, (irExpr)? expr : NULL);	

     PRINT_DOT(fsa, "before BuildEncapFSA return", "buildencapfsa");
        
	return fsa;
}

//needed to implement the % operator
void EncapFSABuilder::CompleteFSA(EncapFSA *fsa, EncapFSA::State *targetState, uint32 index, EncapGraph::GraphNode *targetNode, EncapFSA::Alphabet_t empty, EncapStIterator_t finalNotAccept, EncapFSA::Transition *fromTargetToNotAcpt, PFLTermExpression *expr)
{
	if(!m_Graph.ExistPaths(*targetNode, *targetNode))
	{
		nbASSERT(false,"no path as been found - this case is impossible since CompleteFSA is not invoked if there are not loops involving the targetNode - you have found a bug!");
	}
	AddPath(fsa,targetState,index-1,targetNode, empty, m_Graph.GetMapNodeEdges(), finalNotAccept, fromTargetToNotAcpt,expr);
}

//needed to implement the % operator
void EncapFSABuilder::AddPath(EncapFSA *fsa, EncapFSA::State *targetState, uint32 index, EncapGraph::GraphNode *targetNode, EncapFSA::Alphabet_t empty, multimap<EncapGraph::GraphNode*,EncapGraph::GraphEdge> mp, EncapStIterator_t finalNotAccept, EncapFSA::Transition *fromTargetToNotAcpt, PFLTermExpression *expr)
{
	//targetState is the "start state" of this new part of the automaton... now we link the targetState to a new state related the target protocol, through a number of intermediate nodes
	
	//fromTargetToNotAcpt contains the transition that leads targetState to the final not accepting state. This is needed because, in this part, we have to add labels to this transition

	EncapStIterator_t firstState = fsa->GetStateIterator(targetState);
	EncapFSA::State *auxState = NULL;
	EncapFSA::State *newState = NULL;
	EncapFSA::Transition *auxTransition = NULL;
	
	//when a state related to protocol x has already been created for this part of the automaton, we do not create a copy of it. So, in this map we store the states created in this part
	map<SymbolProto*,EncapFSA::State*> statesCreated;
	
	//we create a map which contains the transitions from each state of this part (except the last) to the final not acceptin state of the automaton
	map<EncapFSA::State*,EncapFSA::Transition*>toFail; 


	//we iterate over the protocols that bring us from the target protocol to the target protocol
	for (EncapGraph::OnPathIterator i = m_Graph.FirstNodeOnPath(); i != m_Graph.LastNodeOnPath(); i++)
	{
		
		//we look in the map if the node related to the current protocol has already been created in this part of the automaton
		map<SymbolProto*,EncapFSA::State*>::iterator isc = statesCreated.find((*i)->NodeInfo);
		
		if(isc == statesCreated.end()){
			//we haven't found the state related to the protocol i, so we create it
			EncapStIterator_t newStateIt = (this->AddState(*fsa, **i, true));
			newState = &(*newStateIt);
			statesCreated[(*i)->NodeInfo] = newState;
			}
		else	
			//we have found the state related to the protocol i
			newState = (*isc).second;
		
		EncapStIterator_t toInsert =  fsa->GetStateIterator(newState);//we need the iterato of the state, and not the state itself
		if ((*i) == targetNode)
		{
			//the current protocol is the target protocol
			if(index == 1)
			{	
				//the current part of the automaton is the last one that we need to create, so the state related the current protocol is the final accepting state
				fsa->setAccepting(toInsert);
				fsa->setFinal(toInsert);
	          		fsa->AddTrans(toInsert, toInsert, empty, true, NULL); 
			}
			else{
				//we are not create the last part of the automaton so, nevertheless the current state is related to the target protocol, it isn't the final accepting state
				auxState = newState; //auxState contains the "start node" of the next part of the automaton
				EncapTrIterator_t newTransition = fsa->AddTrans(toInsert, finalNotAccept, empty, true, NULL);	
				auxTransition = &(*newTransition);	                	
			}               
		}
		
		pair<multimap<EncapGraph::GraphNode*,EncapGraph::GraphEdge>::iterator, multimap<EncapGraph::GraphNode*,EncapGraph::GraphEdge>::iterator> pmp;
		pmp = mp.equal_range(*i);//we get the incoming edges to the node i
		//we iterate over the incaming edges of the node i
		for (multimap<EncapGraph::GraphNode*,EncapGraph::GraphEdge>::iterator mi = pmp.first; mi != pmp.second; ++mi)
		{
			if((*mi).second.From.NodeInfo != targetNode->NodeInfo){
				//if the current edge doesn't come from the target node, i.e. the last node of the previous part of the automaton
				EncapFSA::State *stateFrom = NULL;
				//we look if the state related the from protocol has already been created 
				map<SymbolProto*,EncapFSA::State*>::iterator iscf = statesCreated.find((*mi).second.From.NodeInfo);
				if(iscf == statesCreated.end()){
					//the states doesn't exist, so we create it and store it in the map
					EncapStIterator_t stateFromIt = this->AddState(*fsa,  (*mi).second.From, true);
					stateFrom = &(*stateFromIt);
					statesCreated[(*mi).second.From.NodeInfo] = stateFrom;
				}
				else	
					//the state exists
					stateFrom = (*iscf).second;
				
				EncapLabel_t label = std::make_pair<SymbolProto*, SymbolProto*>((*mi).second.From.NodeInfo, (*i)->NodeInfo);
				EncapStIterator_t toInsertFrom =  fsa->GetStateIterator(stateFrom);
				//we have to see if the "from" state has already a transition to the final not accepting state
				map<EncapFSA::State*,EncapFSA::Transition*>::iterator tf = toFail.find(stateFrom);
				if(tf == toFail.end()){
					//we have to create the transition
					EncapFSA::TransIterator newFailTrans = fsa->AddTrans(toInsertFrom, finalNotAccept, label, true, NULL);
					toFail[&(*toInsertFrom)] = &(*newFailTrans);
				}
				else
				{
					//the transition already exists and we have to add it the new label
					(*tf).second->AddLabel(label);
				}
				
				if(((*i) == targetNode) && (index == 1) && (expr))
				{
					//the current node is the target, we are building the last part of the automata and there is a predicate
					//we have to add an extended transition
					fsa->addExtended(toInsertFrom, label, toInsert, finalNotAccept, expr); 
				}
				else
				{		
					//we add a transition from the "from" to the current state and another one from the "from" state to the final not accepting state
					fsa->AddTrans(toInsertFrom, toInsert, label, false, NULL);
				}
			
			}
			else{
				//the current edge comes from the target node, i.e. the last node of the previous part of the automaton
				EncapLabel_t label = std::make_pair<SymbolProto*, SymbolProto*>(targetNode->NodeInfo, (*i)->NodeInfo);
				if(expr && (index==1) && (*i) == targetNode)
					//if there is a predicate, this is the last part of the automaton and this edge leads to the final accepting state, we must add an extended transition
					fsa->addExtended(firstState, label, toInsert, finalNotAccept, expr);
				else
					//we add a transition
					fsa->AddTrans(firstState, toInsert, label, false, NULL);
				//in both cases, we add a label to the transition that leads from the "start state" to the final not accepting state
				fromTargetToNotAcpt->AddLabel(label);
			}
		} 
	}
	
	if(index>1)
		AddPath(fsa,auxState,index-1,targetNode, empty, m_Graph.GetMapNodeEdges(), finalNotAccept, auxTransition,expr);
...
//HEADER CHAIN
//TODO: supportare i predicati, che sono anche bloccati dalla grammatica
//FIXME: con un set di target funziona? per ora è bloccato dalla grammatica
//FIXME: start in un set ha senso? 

//create the FSA related to a regular expression
//notin any[+,*,^] is not allowed by the grammar. then now we do not consider it
...
{	
	...
	{
		PFLInclusionOperator inclusionOp = (*it)->GetInclusionOperator();
		...
		PFLRepeatOperator repeatOp = (*it)->GetRepeatOperator();	
		newState = CreateLink(fsa,previousProtos,currentProtos,previousState,repeatOp,completeAlphabet);
...
//links two states
EncapFSA::State *EncapFSABuilder::CreateLink(EncapFSA *fsa,set<SymbolProto*>previousProtos,set<SymbolProto*>currentProtos,EncapFSA::State *previousState,PFLRepeatOperator repeatOp,EncapFSA::Alphabet_t completeAlphabet)
{
...
void EncapFSABuilder::FixAutomaton(EncapFSA *fsa)
{
	...
	//add the self loop on the final accepting state
	EncapStIterator_t finalAccept = fsa->GetStateIterator(*(accepting.begin()));
	Alphabet_t empty;
	fsa->AddTrans(finalAccept, finalAccept, empty, true, NULL);	
...
bool EncapFSABuilder::AssignProtocol(EncapFSA *fsa)
{
	bool change = true;
	bool ok = true;
...
//remove the states without exiting or entering transitions.
//obviously, the initial state is removed only in case of it has not exiting transitions
//while the final accepting state is removed only if it has not entering transitions
bool EncapFSABuilder::RemoveUselessStates(EncapFSA *fsa)
{
	bool change = true;
	bool ok = true;
	while(change)
		...
		EncapFSA::StateIterator s = fsa->FirstState();
  		for (; s != fsa->LastState();)
  		{
			...
			if(!(*aux).isAccepting())
			{
				...
				if(transition.size()==0)
				{
					//the state has not outgoing tranistions, then it must be removed
					change = true;
					fsa->removeState(&(*aux));
					continue;
				}
...
bool EncapFSABuilder::ExpandStates(EncapFSA *fsa)
{
	//iterate over the states of teh automaton
	EncapFSA::StateIterator s = fsa->FirstState();
	for (; s != fsa->LastState();)
	{
		...
		//iterate over the exiting transitions. For each symbol, store the detestination state. remove the transition. do not consider the self loop
		map<EncapLabel_t,EncapFSA::State*> labelToState;
		...
		for(list<EncapFSA::Transition*>::iterator t = transitionOut.begin(); t != transitionOut.end();)		
		{
			...
			{
				//this is the self loop, so we do not consider it now
				t++;
				continue;
			}
		...
		}//end iteration over the exiting transitions
		
		//iterate over the incoming transitions. For each symbol, store the destination state. remove the transition.
		Alphabet_t onSelfLoop;
		map<int,map<EncapLabel_t,EncapFSA::State*> > labelFromState;
		map<SymbolProto*,EncapFSA::State*> newStates;
		...
		for(list<EncapFSA::Transition*>::iterator t = transitionIn.begin(); t != transitionIn.end();)		
		{
			...
			for (Alphabet_t::iterator l = labels.begin();l != labels.end();++l)
			{
				if(selfLoop)
					...
					onSelfLoop.insert(*l);
	  				...
	  				currentLabelFromState.insert(pair<EncapLabel_t,EncapFSA::State*>(*l,&(*(*t)->FromState())));

  				if(newStates.count((*l).second)==0)
  				{
  					//a new state related to this protocol does not exist
  					EncapGraph::GraphNode *node = m_Graph.GetNode((*l).second);
  					EncapStIterator_t state = this->AddState(*fsa, *node, true);
  					newStates.insert(pair<SymbolProto*,EncapFSA::State*>((*l).second,&(*state)));
  				}
      	...
      	if((*s).IsInitial())
      	{
      		//since this state is the initial one of the automaton, it must be replaced also by the a state related to startproto, which must
	...
	return RemoveUselessStates(fsa);
}
...
void EncapFSABuilder::ReplaceTransitions(EncapFSA *fsa,map<EncapLabel_t,EncapFSA::State*> labelToState,map<int,map<EncapLabel_t,EncapFSA::State*> > labelFromState,Alphabet_t onSelfLoop,map<SymbolProto*,EncapFSA::State*> newStates)
{
	...
	//replace the incoming transitions
	...
	//replace the outgoing transitions
...
void EncapFSABuilder::FindTraps(EncapFSA *fsa)
{
	...
	RemoveUselessStates(fsa);//we are sure that this method returns true
}

void EncapFSABuilder::LinkToNotAccept(EncapFSA *fsa)
{
