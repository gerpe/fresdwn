...
%type <exp>			ProtocolNoIndex	  
%type <exp>			BoolOperandNoIndex
%type <setExp>		Proto	
...
UnaryExpression:	'(' BoolExpression ')' // DARK cito una modifica di Fulvio, per compatibilità bison-2.4: rimuovo: {$$ = $2}
					...
					|BoolOperand
					|BoolOperand PFL_TUNNELED
								{
									nbASSERT($1->GetType()==PFL_TERM_EXPRESSION,"there should be a bug :-(")
									static_cast<PFLTermExpression*>($1)->SetMandatoryTunnels();
									//if(static_cast<PFLTermExpression*>($1)->GetHeaderIndex()>1)
									//{
										//this case in meaning less
								//		PFL_ERROR(&pfl_lloc, parserInfo, "Header indexing greater than one with \"notunneled\" is meaningless");
								//	}
					...
					|BoolOperandNoIndex PFL_IN InExpression {
						//$1 is a PFLTermExpression
						//$3 is a list<PFLSetExpression*>*
						...
						setExpr->SetInclusionOperator(IN);//the target of the chain is as it were involved in an "in" operation
						$3->push_front(setExpr);
						PFLRegExpExpression *expr = new PFLRegExpExpression($3, REGEXP_OP); 
						if (expr == NULL)
							throw ErrorInfo(ERR_FATAL_ERROR, "MEMORY ALLOCATION FAILURE");
						$$ = expr;
						STORE_EXP_TEXT($$, @$.first_column, @$.first_line, @$.last_column, @$.last_line);
					...
					|BoolOperandNoIndex PFL_NOTIN NotInExpression {	
						//$1 is a PFLTermExpression
						//$3 is a list<PFLSetExpression*>*
...
NotInExpression: /*ProtocolOrSet*/Proto {
				//$1 contains a list<PFLTermExpression*>*
				...
				//we have to create the list of sets
				list<PFLSetExpression*>* setExpList = new list<PFLSetExpression*>();
				setExpList->push_back(/*setExpr*/$1);
				$$ = setExpList;
			}
			| NotInExpression PFL_NOTIN /*ProtocolOrSet*/ Proto {
				//$1 contantains list<PFLSetExpression*>*
				//$3 contains a list<PFLTermExpression*>*
				//PFLSetExpression *setExpr = new PFLSetExpression($3,SET_OP);
				//if (setExpr == NULL)
				//	throw ErrorInfo(ERR_FATAL_ERROR, "MEMORY ALLOCATION FAILURE");
				//setExpr->SetAnyPlaceholder(false);
				/*setExpr*/$3->SetInclusionOperator(NOTIN);
				$1->push_back(/*setExpr*/$3);
				$$ = $1;
			}
...
ProtoOrAny:	/*ProtocolOrSet Repeater {
...
BoolOperandNoIndex:	ProtocolNoIndex
				| RelExpression {
					//we need to chek if the header indexing is 0. Only this case is allowed
					nbASSERT($1->GetType() == PFL_TERM_EXPRESSION, "ops");
					if(static_cast<PFLTermExpression*>($1)->GetHeaderIndex() != 0)
					{
						PFL_ERROR(&pfl_lloc, parserInfo, "In a header chain, the % is not supported");
					}
					$$ = $1;
				};
